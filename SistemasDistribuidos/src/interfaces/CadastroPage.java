					...
					EventQueue.invokeLater(new Runnable() {
						public void run() {
							try {
								...
								JOptionPane.showMessageDialog(null, "Erro ao iniciar a página de login.", "Erro",
	...
	private boolean receberResposta() {
		try {
			...
			if (ValidarJson.verificarCodigo(resposta_servidor)) {
				...
				JOptionPane.showMessageDialog(null, resposta_servidor.get("mensagem").getAsString(), codigo,
						JOptionPane.ERROR_MESSAGE);
				return false;
			}

		} catch (IOException e) {
			JOptionPane.showMessageDialog(null, "Erro ao recever resposta do servidor.", "Erro", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}
}

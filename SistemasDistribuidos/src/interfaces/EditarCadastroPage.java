	...
	public EditarCadastroPage(Socket echoSocket, JsonObject login) {
		...
		btnLogar.addActionListener(e -> {
			...
			if (codigo == 200) {
				...
				if (ValidarJson.verificarMensagem(dados) && dados.has("codigo"))
					JOptionPane.showMessageDialog(null, dados.get("mensagem").getAsString(),
							dados.get("codigo").getAsString(), JOptionPane.ERROR_MESSAGE);
				else
					JOptionPane.showMessageDialog(null, "Json não possui o campo mensagem ou codigo.", "500",
							JOptionPane.ERROR_MESSAGE);
			}
		});
